...
const retrainQueue = new Queue('retrain', { connection: redisConnection });
...
const worker = new Worker('retrain', async (job) => {
    ...
    try {
        ...
        if (process.env.RECOMMENDATION_MODE === 'scratch') {
            await axios.post(`${ml_service_url}/train`, {}, { timeout: 30000, headers: { 'Content-Type': 'application/json' } });
